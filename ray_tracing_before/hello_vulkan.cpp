...
auto HelloVulkan::objectToVkGeometryKHR(const ObjModel& model){
  ...
  VkDeviceAddress indexAddresss= nvvk::getBufferDeviceAddress(m_device,model.indexBuffer.buffer);
  ...
  triangles.indexData.deviceAddress = indexAddresss;
  triangles.maxVertex = model.nbVertices;

  VkAccelerationStructureGeometryKHR asGeom = nvvk::make<VkAccelerationStructureGeometryKHR>();
...
void HelloVulkan::createRtDescriptorSet(){
  m_rtDescSetLayoutBind.addBinding(RtxBindings::eTlas,VK_DESCRIPTOR_TYPE_ACCELERATION_STRUCTURE_KHR,1,VK_SHADER_STAGE_RAYGEN_BIT_KHR,nullptr);
  ...
  VkDescriptorSetAllocateInfo allocateInfo = nvvk::make<VkDescriptorSetAllocateInfo>();
  ...
  VkWriteDescriptorSetAccelerationStructureKHR descASInfo = nvvk::make<VkWriteDescriptorSetAccelerationStructureKHR>();
...
void HelloVulkan::createRtPipeline(){
  ...
  stage.pName = "main";
  stage.module = nvvk::createShaderModule(m_device,nvh::loadFile("spv/raytrace.regen.spv",true,defaultSearchPaths,true));
  ...
  group.generalShader = VK_SHADER_UNUSED_KHR;
  group.closestHitShader = eCloseHit;
  m_rtShaderGroups.push_back(group);
